...
def login():
    try:
        ...
        print(errmsg)
        return redirect(url_for('com_bp.showFailedPage', errormessage="Can't connect to FOLLOWUP.DB!"))
...
def homePage():
    ...
    if userinfo["Rights"] == 'user':
        ...
        errormessage = "Hey, don't hack it!"
        return redirect(url_for('com_bp.showFailedPage', errormessage=errormessage))
...
def showimportexport():
    data = user.session_info()

    if data["Rights"] == "proofreader":
        tag_status = "disabled"
    else:
        tag_status = ""

    context = {"tag_status": tag_status}
